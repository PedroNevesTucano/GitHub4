...
class MatchDecks {
    ...
    static async playDeckCard(game, deckId) {
        ...
        try {
            ...
            if (num == 1 && chosenClass == 1 && playerObj.state.name ) {
            ...
            where hus_ugc_user_game_id = ? and hus_ugc_id = ? and hus_ugc_active=1`,
                ...
                if (playerObj.state.name == "Mirror") {
                    return { status: 400, result: { msg: "Player cannot do any other actions while on mirror state" } };
                }
                // verifications done, set card to inactive
